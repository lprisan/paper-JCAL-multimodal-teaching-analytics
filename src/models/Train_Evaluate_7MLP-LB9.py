...
# Usage: Train_Evaluate_7MLP-LB9.py <label> <target-variable> <data-sources> <train-set-sessions> <test-set-sessions>
...
if(len(sys.argv)!=6):
  sys.exit("Wrong number of arguments. Usage:\nTrain_Evaluate_7MLP-LB9.py <label> <target-variable> <data-sources> <train-set-sessions> <test-set-sessions>")
...
def create_deeper_dropout_decay_PCA(n_components, lookback, learningrate, n_epochs, n_outs):
    # create model
    model = Sequential()
    ...
    model.add(Dense(20, init='uniform', activation='tanh'))
    model.add(Dropout(0.2))
    model.add(Dense(n_outs, init='uniform', activation='sigmoid'))
...
# Reshape the dataset with the lookback
X_train_lb, Y_train_lb = create_dataset(train[['session','timestamp']].values, X_train_pca, dummy_y_train, lookbk)
X_test_lb, Y_test_lb = create_dataset(test[['session','timestamp']].values, X_test_pca, dummy_y_test, lookbk)
print X_train_lb.shape, Y_test_lb.shape

#model = create_baseline()
model = create_deeper_dropout_decay_PCA(n_components=k, lookback=lookbk, learningrate=learningrate, n_epochs=n_epochs, n_outs = outs)
#print model.summary()
#############################################################################
...
# Accuracy
print('Accuracy:')
acc = accuracy_score(numpy.argmax(Y_test_lb, axis=1), numpy.argmax(Y_pred, axis=1))
print(acc)
