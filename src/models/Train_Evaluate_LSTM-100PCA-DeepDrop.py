...
model = create_LSTM_DeepDrop(hidden=100, lstm_layers=3, input_dim = k, output_dim = outs, mlp_layers=2, dropout=0.3)
#print model.summary()
#############################################################################
...
history = model.fit(X_train_pca_reshaped, dummy_y_train_reshaped, validation_data=(X_test_pca_reshaped,dummy_y_test_reshaped),
                    nb_epoch=1000, batch_size=16, verbose=0, callbacks=callbacks_list)
